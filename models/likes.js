...
exports.like = async function like (id, uid) {
  let query = `INSERT INTO dogslikes (dogsID,userID) VALUES (${id},${uid}) ON CONFLICT ON CONSTRAINT  NoDuplicateLike  
DO NOTHING;`   
  try{
...
exports.findLikeCheck = async function findLikeCheck(id,uid) {
  let query = `SELECT * FROM dogslikes WHERE  dogsID=${id} AND userid=${uid} `
  try{
    await db.run_query(query, [id, uid]);  
...
exports.dislike = async function dislike (id, uid) {
  let query = "DELETE FROM dogslikes WHERE dogID=? AND dogsID=?; ";
   try{
    await db.run_query(query, [id, uid]);  
...
exports.count = async function count (id) {
  let query = "SELECT count(1) as likes FROM dogslikes WHERE dogsID=?;";
